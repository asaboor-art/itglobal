<template>
    ...
    <div class="card card-secondary card-outline">
        <div class="card-header d-flex justify-content-end">
            <a class="btn btn-secondary" href="/admin/headers/create">
                {{ Lang.add_new_msg.replace(':attribute',Lang.header) }}
            </a>
...
mounted(){
            ...
            dropdown:[
                {
                    ...
                    action:(row) =>{
                        ...
                        window.location.href = '/admin/headers/edit/'+row.id
                    }
                },
                {
                    name:(row) => {
    ...
    this.getHeaders(this.pageNo,this.pageSize,this.filter);
},
...
methods:{
    ...
    async getHeaders(){
        ...
        const { headers, getHeaders } = useHeaders();
        this.loader = true
        let ref = this

        try{
            await getHeaders(ref.pageNo,ref.pageSize,ref.orderBy,ref.order,ref.filter);
            ref.data = headers
            ref.loader = false;
        }catch(e){
            ref.loader = false;
        ...
        let ref = this
            confirmAlert(async () => {
                this.loader = true
                    ...
                    ref.getHeaders(ref.pageNo,ref.pageSize,ref.filter);
            })
    },
    async _delete(id){
        ...
        let ref = this
            confirmAlert(async () => {
                this.loader = true
                    ...
                    await _delete(id)
                    ref.getHeaders(ref.pageNo,ref.pageSize,ref.filter);
                    successAlert(Language.success_msg.replace(':attribute',Language.header).replace(':action',Language.deleted))
