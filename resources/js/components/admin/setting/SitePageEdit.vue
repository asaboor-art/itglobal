...
<template>
    <div>
      ...
      <div class="container-fluid">
        <div class="row mb-2">
          <div class="col-sm-6">
            <h1>{{ Language.site_page }} {{ Language.edit }}</h1>
          </div>
          <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
              <li class="breadcrumb-item"><a href="/admin">{{ Language.dashboard }}</a></li>
              <li class="breadcrumb-item"><a href="/admin/general-setting/site-page">{{ Language.site_page }}</a></li>
...
export default {
   ...
   async mounted(){
        ...
        const {sitePage,getSitePage} = useSetting();
        await getSitePage(this.id);
        console.log(sitePage);
        this.form.title = sitePage.value.title;
        this.form.slug = sitePage.value.slug;
        this.form.description = sitePage.value.description;
    },
    methods:{

        async update(){
            const {updateSitePage,errors} = useSetting();
            const {errorAlert,successAlert} = useService();
            this.errors = [];
            let ref = this;
            await updateSitePage(this.form,this.id).then(async (response) => {
